...
export default function EditRound() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setGrupal((r as any).grupal ?? false)
      setCompetitionName((r as any).competition_name ?? '')
      setSideMatch(((r as any).side_match ?? 'none'))
      setSideHcpPct((r as any).side_hcp_pct ?? 100)
  ...
  // Stroke Grupal + Grupal=Sí: hereda el nombre del primer grupal del día en la
  // misma cancha (merge por cancha + fecha + modalidad). Excluye esta misma partida.
  useEffect(() => {
    ...
    supabase.from('rounds')
      ...
      .eq('course_id', courseId).eq('date', date).neq('id', id)
  ...
  const isValid = () => {
    ...
    if (mode === 'stroke_grupal' && grupal && !competitionName.trim()) return false
  ...
  const handleSave = async () => {
    ...
    if (mode === 'stroke_grupal') {
      update.grupal = grupal
      update.competition_name = grupal ? competitionName.trim() : null
      update.side_match = sideMatch
      update.side_hcp_pct = sideHcpPct
    }
  ...
  const isDoublesMode = mode === 'combinado_4' || mode === 'matchplay_dobles' || mode === 'mejor_peor_suma' || (mode === 'stroke_grupal' && sideMatch === 'dobles')
      ...
      <main style={{ maxWidth: 640, margin: '0 auto', padding: '24px 16px', display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <section style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: 20 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#1B4332' }} />
        ...
        <section style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {players.map((p, i) => (
              <div key={p.id} style={{ display: 'flex', gap: 10, alignItems: 'center', padding: '10px 14px', borderRadius: 10, background: 'rgba(27,42,26,0.03)', border: '1px solid var(--border)' }}>
                ...
                {isDoublesMode && (
                  <select value={p.team} onChange={e => updatePlayer(i, 'team', parseInt(e.target.value))}
                    ...
                    <option value={1} style={{ background: '#EDEAD8' }}>Team 1</option>
                    <option value={2} style={{ background: '#EDEAD8' }}>Team 2</option>
                  </select>
                )}
              </div>
            ))}
        ...
        <section style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {players.map((p, i) => (
              <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', borderRadius: 10, background: 'rgba(27,42,26,0.03)', border: '1px solid var(--border)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 12, color: lockedName ? '#2B5F7A' : 'var(--text3)', marginTop: 6, lineHeight: 1.4 }}>
                    {lockedName
                      ...
                      : 'Usa el mismo nombre/cancha/fecha en los demás cuartos para que se junten en el leaderboard.'}
                  </p>
                </div>
              )}
              ...
              <div style={{ padding: 14, background: 'rgba(184,147,90,0.08)', borderRadius: 10, border: '1px solid #f59e0b20' }}>
                ...
                <p style={{ fontSize: 12, color: 'var(--text3)', marginTop: 8, lineHeight: 1.4 }}>
                  Los partidos paralelos corren en match play y no afectan el leaderboard.
