  ...
  return (
    <div style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 10, padding: '12px 14px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
        ...
        {match && (() => {
          const winnerColor = match.leaderId === p1.id ? PLAYER_COLORS[pi1] : match.leaderId === p2.id ? PLAYER_COLORS[pi2] : 'var(--text3)'
          return <div style={{ fontFamily: 'var(--display)', fontSize: 20, color: winnerColor, minWidth: 60, textAlign: 'center' }}>{match.status}</div>
        })()}
        {stroke && (
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <main style={{ maxWidth: 800, margin: '0 auto', padding: '16px 12px' }}>
                  ...
                  <table style={{ borderCollapse: 'collapse', width: '100%', minWidth: 500 }}>
                    ...
                    <tbody>
                      {players.map((p, pi) => {
                        ...
                        const totalAll = holes.reduce((s, h) => s + (getScore(p.id, h.hole_number) || 0), 0)
                        ...
                        return (
                          <tr key={p.id}>
                            ...
                            <td style={{ padding: '6px 8px', border: '1px solid var(--border)', background: 'var(--surface)', textAlign: 'center', fontWeight: 700, color: PLAYER_COLORS[pi], fontSize: 14 }}>
                              {isLastSet
                                ? (totalAll > 0 ? <span style={{ color: '#2dd4bf' }}>{totalAll}</span> : '—')
                                : (rowTotal > 0 ? rowTotal : '—')}
                            </td>
                          </tr>
                        )
        ...
        {/* ── SCORE NETO ── */}
        {activeTab === 'neto' && (() => {
          // Helper: renders a net score table for a group of players with a given pct and strategy
          // strategy: 'individual' = getExtraStrokes (Stroke Play), 'relative' = getRelativeExtra (Match/Bismarck)
          const NetTable = ({ title, titleColor, groupPlayers, pct, strategy }: {
            title: string, titleColor: string,
            groupPlayers: typeof players,
            pct: number,
            strategy: 'individual' | 'relative'
          }) => {
            const allHcps = groupPlayers.map(p => p.handicap)
            return (
              <div style={{ marginBottom: 24 }}>
                <div style={{ fontFamily: 'var(--display)', fontSize: 12, letterSpacing: 2, color: titleColor, paddingBottom: 8, display: 'flex', gap: 8, alignItems: 'center' }}>
                  <span>{title}</span>
                  <span style={{ color: 'var(--text3)', fontSize: 11 }}>· HCP {pct}%{pct === 0 ? ' (sin ajuste)' : ''}</span>
                </div>
                {[front9, ...(back9.length > 0 ? [back9] : [])].map((holeSet, si) => {
                  const isLastSet = si === (back9.length > 0 ? 1 : 0)
                  return (
                      ...
                      <table style={{ borderCollapse: 'collapse', width: '100%', minWidth: 500 }}>
                        <thead>
                          <tr>
                            ...
                            <th style={{ width: 52, padding: '6px 8px', fontSize: 11, color: '#2dd4bf', background: 'var(--surface2)', border: '1px solid var(--border)' }}>{isLastSet ? 'TOTAL' : 'IDA'}</th>
                          </tr>
                        </thead>
                        <tbody>
                          {groupPlayers.map(p => {
                            const pi = players.findIndex(pl => pl.id === p.id)
                            let rowNeto = 0; let hasAll = true
                            // Total neto de los 18 hoyos (para mostrar en última vuelta)
                            let totalNeto = 0; let hasAllTotal = true
                            holes.forEach(h => {
                              const s = getScore(p.id, h.hole_number)
                              ...
                              if (s !== null) totalNeto += s - extra; else hasAllTotal = false
                            })
                            return (
                              <tr key={p.id}>
                                <td style={{ padding: '6px 10px', border: '1px solid var(--border)', background: 'var(--surface)' }}>
                                ...
                                <td style={{ padding: '6px 8px', border: '1px solid var(--border)', background: 'var(--surface)', textAlign: 'center', fontWeight: 700, fontSize: 14 }}>
                                  {isLastSet
                                    ? (hasAllTotal ? <span style={{ color: '#2dd4bf' }}>{totalNeto}</span> : '—')
                                    : (hasAll ? <span style={{ color: PLAYER_COLORS[pi] }}>{rowNeto}</span> : '—')}
                                </td>
                              </tr>
                            )
                          })}
          ...
          // ── Ryder: 1 tabla dobles + 4 tablas individuales ──
          if (r.mode === 'combinado_4') {
            ...
            return (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                <div style={{ background: 'var(--surface)', border: '1px solid #34d39920', borderRadius: 12, padding: '14px 14px 4px' }}>
                  <NetTable
                    title={`DOBLES — ${dMode === 'stroke' ? 'STROKE' : 'MATCH PLAY'}`}
                    titleColor="#34d399"
                    groupPlayers={players}
                    pct={dPct}
                    strategy={dMode === 'stroke' ? 'individual' : 'relative'}
                  />
                </div>
                {/* 4 individuales */}
                {pairs.map(([p1, p2], idx) => {
                  ...
                  return (
                    <div key={idx} style={{ background: 'var(--surface)', border: '1px solid #f59e0b20', borderRadius: 12, padding: '14px 14px 4px' }}>
                      <NetTable
                        title={`${p1.name} vs ${p2.name} — ${iMode === 'stroke' ? 'STROKE' : 'MATCH PLAY'}`}
                        titleColor={PLAYER_COLORS[pi1]}
                        groupPlayers={[p1, p2]}
                        pct={iPct}
          ...
          // ── Bismarck + Individuales: tabla bismarck (relativa) + 3 individuales ──
          if (r.mode === 'combinado_bismarck') {
            const bPct = (r as any).bismarck_hcp_pct ?? 100
            const iPct = (r as any).individual_hcp_pct ?? 80
            const iMode = (r as any).individual_mode || 'matchplay'
            const pairs = [
              [players[0], players[1]], [players[0], players[2]], [players[1], players[2]]
            ]
            return (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          // ── Modos simples: una sola tabla ──
          // Stroke Play → individual (ventaja vs el campo)
          // Match Play / Dobles / Bismarck → relative (ventaja entre jugadores)
          const simpleStrategy = r.mode === 'stroke' ? 'individual' : 'relative'
          return (
            <>
              <NetTable title="" titleColor="var(--text3)" groupPlayers={players} pct={hcpPct} strategy={simpleStrategy} />
              <div style={{ fontSize: 11, color: 'var(--text3)', paddingTop: 4 }}>El punto verde indica ventaja en ese hoyo.</div>
            </>
          )
        })()}
